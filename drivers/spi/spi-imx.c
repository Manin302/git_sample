...
#define SPI_GPIO_BUS_NUM        0

#define SPI_GPIO_SCK            121
#define SPI_GPIO_MOSI           123 /* set gpio 3 as MOSI */
#define SPI_GPIO_MISO           124 /* set gpio 4 as MISO */

#define SPI_GPIO_CHIP_SELECT_SPIDEV             0
#define SPI_GPIO_NUM_CHIPSELECT                 1
#define SPI_GPIO_CONTROLLER_DATA_SPIDEV         (void *) 76 
...
static struct platform_device *bsp_spi_gpio_devs[] __initdata = {
        &bsp_spi_gpio_device,
};
...
static int __init spi_gpio_device_init(void)
{
        ...
        /* core init */
        printk("INFO: initializing spi_gpio ...\n");

        ret = platform_add_devices(bsp_spi_gpio_devs,
                ARRAY_SIZE(bsp_spi_gpio_devs));
        if (ret < 0) {
                printk("ERROR: unable to add spi_gpio\n");
                return ret;
        }

        spi_register_board_info(rts_spi_gpio_board_info,
                ARRAY_SIZE(rts_spi_gpio_board_info));
